...
class Tes_kerjakan extends Tes_Controller {
        ...
        if(!empty($tes_id)){
            ...
            if($query_tes->num_rows()>0){
                ...
                if($tanggal>=$tanggal_tes){
                    ...
                    // Mengambil data per soal
                    $teslog = $this->cbt_tes_soal_model->get_by_testuser_limit($query_tes->tesuser_id, 1)->row();
                    $data_soal = $this->get_soal($teslog->tessoal_id, $query_tes->tesuser_id);

                    $data['tes_soal'] = $data_soal['tes_soal'];
                    $data['tes_log_id'] = $teslog->tessoal_id;
                    $data['tes_soal_nomor'] = $teslog->tessoal_order;
    ...
    function simpan_jawaban(){
        ...
        $this->form_validation->set_rules('tes-log-id', 'Soal','required|strip_tags');
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $tes_log_id = $this->input->post('tes-log-id', TRUE);
            ...
            if($this->cbt_tes_user_model->count_by_status_waktu($tes_user_id)->row()->hasil>0){
                ...
                $query_soal = $this->cbt_tes_soal_model->get_by_teslog_limit($tes_log_id, 1);
                if($query_soal->num_rows()>0){
                    $query_soal = $query_soal->row();

                    $data_tes_log['tessoal_change_time'] = date('Y-m-d H:i:s');
                    ...
                    if($query_soal->soal_tipe==1){
                        ...
                        $query_jawaban = $this->cbt_tes_soal_jawaban_model->get_by_teslog_answer($tes_log_id, $jawaban)->row();

                        // Mengupdate pilihan jawaban benar
                        $data_jawaban['soaljawaban_selected']=1;
                        $this->cbt_tes_soal_jawaban_model->update_by_teslog_answer($tes_log_id, $jawaban, $data_jawaban);
                        // Mengupdate pilihan jawaban salah
                        $data_jawaban['soaljawaban_selected']=0;
                        $this->cbt_tes_soal_jawaban_model->update_by_teslog_answer_salah($tes_log_id, $jawaban, $data_jawaban);

                        // Mengupdate score, change time jika pilihan benar
                        if($query_jawaban->jawaban_benar==1){
                            $data_tes_log['tessoal_nilai'] = $query_tes->tes_score_right;
                        }else{
                            $data_tes_log['tessoal_nilai'] = $query_tes->tes_score_wrong;
                        }

                        $this->cbt_tes_soal_model->update('tessoal_id', $tes_log_id, $data_tes_log);
                        ...
                        $data_tes_log['tessoal_jawaban_text'] = $jawaban;
                        $data_tes_log['tessoal_nilai'] = 0;
                        $this->cbt_tes_soal_model->update('tessoal_id', $tes_log_id, $data_tes_log);
    ...
    /**
     * Mendapatkan soal dengan output json
     */
    function get_soal_by_teslog($teslog_id=null, $tesuser_id=null){
        $data['data'] = 0;
        if(!empty($teslog_id) AND !empty($tesuser_id)){
            $data_soal = $this->get_soal($teslog_id, $tesuser_id);
            $data['data'] = $data_soal['data'];
            if(!empty($data_soal['tes_soal'])){
                $data['tes_soal'] = $data_soal['tes_soal'];
                $data['tes_log_id'] = $data_soal['tes_log_id'];
     ...
     * @param      <type>  $teslog_id  The teslog identifier
     *
     * @return     string  The soal.
     */
    private function get_soal($teslog_id=null, $tesuser_id=null){
        $data['tes_log_id'] = '';
        $data['tes_soal'] = '';
        $data['data'] = 0;
        if(!empty($teslog_id) AND !empty($tesuser_id)){
            ...
            if($this->cbt_tes_user_model->count_by_status_waktu($tesuser_id)->row()->hasil>0){
                $data['data'] = 1;
                $query_soal = $this->cbt_tes_soal_model->get_by_teslog_limit($teslog_id, 1);
                $soal = '';
                if($query_soal->num_rows()>0){
                    $data['tes_log_id'] = $teslog_id;

                    $query_soal = $query_soal->row();

                    // Mengupdate tessoal_display_time pada table test_log
                    $data_tes_log['tessoal_display_time'] = date('Y-m-d H:i:s');
                    $this->cbt_tes_soal_model->update('tessoal_id', $teslog_id, $data_tes_log);
                    ...
                    if($query_soal->soal_tipe==1){
                        $query_jawaban = $this->cbt_tes_soal_jawaban_model->get_by_teslog($query_soal->tessoal_id);
                        ...
                        if(!empty($query_soal->tessoal_jawaban_text)){
                            $soal = $soal.'<textarea class="textarea" id="soal-jawaban" name="soal-jawaban" style="width: 100%; height: 150px; font-size: 13px; line-height: 25px; border: 1px solid #dddddd; padding: 10px;">'.$query_soal->tessoal_jawaban_text.'</textarea>
                                <button type="button" onclick="jawab()" class="btn btn-default" style="margin-bottom: 5px;" title="Simpan Jawaban">Simpan</button>
                                ';
                        }else{
    ...
    function update_status_audio($teslog_id=null){
        $data['data'] = 0;
        if(!empty($teslog_id)){
            $data['data'] = 1;
            $data_tes['tessoal_audio_play'] = 1;
            $this->cbt_tes_soal_model->update('tessoal_id ', $teslog_id, $data_tes);
